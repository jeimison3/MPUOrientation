...
void convert() {
  ...
  /* Convert the gyro y-axis */
  if (stringPitch3 != null) {
    stringPitch3 = trim(stringPitch3); // Trim off any whitespace
    pitch3[pitch3.length - 1] = scrPitch - map(-float(stringPitch3), minAngle*2, maxAngle*2, 0, height*kMulti); // Convert to a float and map to the screen height, then save in buffer
  }
  ...
  /* Convert the accelerometer y-axis */
  if (stringRoll3 != null) {
    stringRoll3 = trim(stringRoll3); // Trim off any whitespace
    roll3[roll3.length - 1] = scrRoll - map(float(stringRoll3), minAngle, maxAngle, 0, height*kMulti); // Convert to a float and map to the screen height, then save in buffer
  }
  ...
  /* Convert the complementary filter x-axis */
  if (stringYaw3 != null) {
    stringYaw3 = trim(stringYaw3); // Trim off any whitespace
    yaw3[yaw3.length - 1] = scrYaw - map(float(stringYaw3), minAngle*2, maxAngle*2, 0, height*kMulti); // Convert to a float and map to the screen height, then save in buffer
  }
